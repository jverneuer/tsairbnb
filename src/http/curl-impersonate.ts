...
export class CurlImpersonateClient implements HttpClient {
  async request(req: HttpRequest): Promise<HttpResponse> {
    ...
    const { status, stdout, cookieHeader } = await run(BINARY, args, req.body, getConfig().timeoutMs);

    // curl-impersonate writes Set-Cookie to a jar (-c flag); parse it.
    const cookies = cookieHeader ? parseCookieJar(cookieHeader) : {};

    return { status, body: stdout, cookies };
  }
...
export function run(
  ...
  timeoutMs: number,
): Promise<{ status: number; stdout: string; cookieHeader: string }> {
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      ...
      const { status, body: clean, cookies } = demux(stdout);
      resolve({ status, stdout: clean, cookieHeader: cookies });
    });
  });
}
...
export function demux(stdout: string): { status: number; body: string; cookies: string } {
  ...
  const body = sep >= 0 ? stdout.slice(sep + 4).replace(/__HTTP_STATUS__:\d+\s*$/, "") : stdout;
