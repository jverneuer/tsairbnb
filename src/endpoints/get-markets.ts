...
export type GetMarketsMode =
  | { mode: "live"; apiKey: string; language?: string }
...
export async function getMarkets(opts: GetMarketsMode): Promise<Envelope<unknown[]>> {
  ...
  const url = `https://www.airbnb.com/api/v2/user_markets?locale=${locale}&currency=${currency}&language=en`;
  const t0 = Date.now();
  ...
  const terms = (raw as any)?.user_markets ?? (raw as any)?.data?.user_markets ?? raw;
  return { ok: true, data: Array.isArray(terms) ? terms : [terms], raw, meta: { fetchedAt: new Date().toISOString(), endpoint: "get-markets", durationMs: Date.now() - t0, parserVersion: "passthrough", warnings: [], mode: "live" } };
}
